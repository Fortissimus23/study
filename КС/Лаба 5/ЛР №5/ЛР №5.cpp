...
{
	//Создать файл сервиса, манипклятора для выполнения icmp запросов
	...
	for (unsigned int c = 0; c < RequestCount; c++)
	{
		...
		(
			...
			SendData,
			//укзательнабуфферэхо-запроса

			sizeof(SendData),
			//размер буффера эхо-запроса

			(PIP_OPTION_INFORMATION)&option,
			//укзатель на структуру с опициями запроса

			pIpe,
			//укзательнабуфферэхо-ответа

			sizeof(ICMP_ECHO_REPLY) + sizeof(SendData),
			//размер буффера эхо-ответа
...
int main(int argc, char** argv)
{
	...
	{*/
		Ping("87.240.137.158", Timeout, RequestCount);
	//}
